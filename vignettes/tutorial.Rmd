...
\forceindent Each node $i \in V$ is assigned to a couple of variables $(p_i, d_i)$, which has the following meaning:
...
Should we introduce the auxiliary function $f_{ij} = \mathbbm{1}_{p_i=j \Rightarrow d_i=d_j+1\wedge p_j\neq\{*\}} = 1 - \delta_{p_i, j}\left[1-\delta_{d_i,d_j+1}\left(1-\delta_{p_j,*}\right)\right]$, the constraints will take the form:
...
\forceindent Let $c_{i*} = \lambda b_i$, then the Problem 3.1 can be rewritten as follows:

$$
\min\{\mathcal{H}(\mathbf{p}): (\mathbf{d}, \mathbf{p}) \in \mathcal{T}\}
\tag{3.3}
$$

where $\mathbf{d} = \{d_i\}_{i \in V}$, $\mathbf{p}=\{p_i\}_{i \in V}$, $\mathcal{T} = \{(\mathbf{d}, \mathbf{p}): g_{ij} = 1\ \forall (ij) \in E\}$ and $\mathcal{H}(\mathbf{p}) = \sum_{i \in V} c_{ip_i}$.

\forceindent The corresponding probability distribution is:
...
## 3.2 Message-passing equations

\forceindent In this section we are going to introduce message update rules for loop-free graphs and then generalise the equations. Using the previous results the following holds:
...
\max_{d_i, p_i: g_{ij}=1}\left\{
    - c_{ip_i}
    + \sum_{k \in \partial i\setminus j} m_{k\rightarrow i} (d_i, p_i)
\right\}
\tag{3.5}
$$
...
\noindent Lets introduce new variables: $A_{ij}^{d} = \max_{p_i\neq \left\{j, *\right\}}\left\{.\right\}|_{d_i = d}$, $B^d_{ij} = \max_{p_i=*}\left\{.\right\}|_{d_i = d}$, $C_{ij}^d = \max_{p_i=j}\left\{.\right\}|_{d_i = d}$ Hence:
...
\begin{cases}
A_{ij}^{d_j-1}                             & p_j = i\\
\max_{d_i}\max[A_{ij}^{d_i}; B_{ij}^{d_i}] & p_j = *\\
\max\left[
C_{ij}^{d_j+1};
\max_{d_i}
\max[A_{ij}^{d_i};
...
A_{ij}^{d_j-1}                            & p_j = i\\
D_{ij}                                    & p_j = *\\
\max\left[ C_{ij}^{d_j+1}; D_{ij} \right] & p_j \neq \left\{i, *\right\}
\end{cases}\\
&=
\begin{cases}
A_{ij}^{d_j-1}      & p_j = i\\
...
\noindent Now we need to derive update rules for $A, B, C$:
...
\max_{p_i\neq \left\{j, *\right\}}\left\{
    - c_{ip_i}
    + \sum_{l \in \partial i\setminus j} m_{l\rightarrow i} (d, p_i)
\right\}\\
&=
\max_{k\in\mathbf{Nb}(i)\setminus j}\left\{
    - c_{ik}
    + \sum_{l \in \partial i\setminus j} m_{l\rightarrow i} (d, k)
\right\}\\
&=
\sum_{k\in\mathbf{Nb}(i)\setminus j} E_{kj}
+
\max_{k\in\mathbf{Nb}(i)\setminus j}\left\{
    - c_{ik} - E^d_{kj} + A^{d-1}_{kj}
\right\}
\end{aligned}
\tag{3.8}
$$
...
$$
\begin{aligned}
B^d_{ij} = \max_{p_i=*}\left\{.\right\}|_{d_i = d}
&=
- c_{i*} + \sum_{k \in \partial i\setminus j} m_{k\rightarrow i} (d, *)\\
& =
-c_{i*} + \sum_{k \in \partial i\setminus j}D_{kj}
\end{aligned}
\tag{3.9}
$$
...
$$
\begin{aligned}
C_{ij}^d = \max_{p_i=j}\left\{.\right\}|_{d_i = d}
&=
- c_{ij} + \sum_{k \in \partial i\setminus j} m_{k\rightarrow i} (d, j) \\
&=
- c_{ij} + \sum_{k \in \partial i\setminus j} E_{kj}^d\\
\end{aligned}
\tag{3.10}
$$
