...
\noindent Lets introduce new variables: $A_{i\rightarrow j}^{d} = \max_{p_i\neq \left\{j, *\right\}}\left\{.\right\}|_{d_i = d}$, $B^d_{i\rightarrow j} = \max_{p_i=*}\left\{.\right\}|_{d_i = d}$, $C_{i\rightarrow j}^d = \max_{p_i=j}\left\{.\right\}|_{d_i = d}$ Hence:
...
Marginal distribution can be found as follows
...
&=
\begin{cases}
- c_{ji} + \sum_{k \in \mathbf{Nb}(i)} m_{k \rightarrow j} (d_j, i),    & p_j = i \in \mathbf{Nb}(j)\\
- c_{j*} + \sum_{k \in \mathbf{Nb}(i)} m_{k \rightarrow j} (d_j, *),    & p_j = \{*\}\\
\end{cases}\\
&=
\begin{cases}
\sum_{k\in\mathbf{Nb}(i)} E_{k\rightarrow j}^d + \left(- c_{ji} - E^d_{i\rightarrow j} + A^{d-1}_{i\rightarrow j}\right),   & p_j = i \in \mathbf{Nb}(j)\\
- c_{j*} + \sum_{k \in \mathbf{Nb}(i)} D_{k \rightarrow j},                                                                 & p_j = \{*\}\\
\end{cases}\\
&=
\begin{cases}
F_{j \rightarrow i},   & p_j = i \in \mathbf{Nb}(j)\\
G_{j \rightarrow i},   & p_j = \{*\}\\
\end{cases}\\
\end{aligned}
$$
