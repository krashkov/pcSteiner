...
\forceindent Each node $i \in V$ is assigned a couple of variable $(p_i, d_i)$, which has the following meaning:
...
\forceindent The loopy belief propagation update rule for messages (in max-sum form) is as follows:

$$
m_{i\rightarrow j}^{t+1} (d_j, p_j)
=
\max_{d_i, p_i: g_{ij}=1}\left\{
    - c_{ip_i}
    + \sum_{k \in \partial i\setminus j} m^t_{ki} (d_i, p_i) + C_{ij}
\right\}
$$
